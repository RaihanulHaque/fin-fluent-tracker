...
const ExpenseInput: React.FC<ExpenseInputProps> = ({ onAddExpense }) => {
  ...
  const parseExpense = (text: string): Transaction[] => {
    const patterns = [
    ...
    while ((match = regex.exec(text)) !== null) {
      ...
      const type: 'income' | 'expense' = ['salary', 'freelance', 'bonus', 'income'].includes(description.toLowerCase()) ? 'income' : 'expense';
  ...
  return (
    <div className="space-y-4">
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
        <p className="text-blue-800 text-sm mb-2 flex items-center gap-2">
